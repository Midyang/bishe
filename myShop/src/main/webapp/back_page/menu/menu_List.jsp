...
<script type="text/javascript">
	function fenye(curPage){
		...
		location.href="menu/list?curPage="+curPage+"&mohuName="+name;
	};
	function mohu() {
		var name=document.getElementById("mohu").value;
		location.href="menu/list?curPage=1&mohuName="+name;
	};
	function selectById(id) {
		location.href="menu/selectById?id="+id;
	};
	function deleteById(id,currPage) {
		if(confirm("你确认删除吗?")){
			var name=document.getElementById("mohu").value;
			location.href="menu/deleteById?id="+id+"&curPage="+currPage+"&mohuName="+name;
		}
	};
	function add(){
		location.href="menu/addBeforeSel";
	};
	function showColor(id){
		$("#classOne"+id).css("background-color","#FFEEFF");
	};
	function closeColor(id){
		$("#classOne"+id).css("background-color","#FFFFFF");
	};
</script>
<title>目录管理</title>
</head>
<body>
		...
		<div id="start">
			...
			<div id="head">
				<!-- 标题**导航部分  -->
				<div id="head_text">
				目录列表 &lt; <a href="back_page/hello.jsp">&nbsp;首页</a>
				</div>
				<!-- 退出登陆部分  -->
			</div>
			<!-- 根据名字模糊查询  部分-->
					...
					<table id="main_table"  cellpadding="0" cellspacing="0">
	                    ...
	                    -->
							<c:forEach items="${list }" var="l">
		                    	<tr id="classOne${l.id}" onmouseover="showColor('${l.id}')" onmouseout="closeColor('${l.id}')" >
									<td class="table_tr_col">${l.id }</td>
									<td class="table_tr_col">${l.menuName }</td>
									<td class="table_tr_col">${l.menuURL }</td>
									<td class="table_tr_col">${l.parentMenuName }</td>
