...
<body>
		...
		<div id="start">
			...
			<div id="head">
				<!-- 标题**导航部分  -->
				<div id="head_text">
				添加目录项  &lt; <a href="menu/list">&nbsp;目录列表</a> &lt; <a href="back_page/hello.jsp">&nbsp;首页</a>
				</div>
				<!-- 退出登陆部分  -->
			</div>
			<!-- 表格-->
			<div id="main">
					<!-- 要提交的表单-->
					<form action="menu/add" method="post" onsubmit="return verify()">
						<!-- 表格-->
						<table id="main_table" border="0" cellpadding="0" cellspacing="0">
							...
							<tr class="table_tr">
								...
								<td >
									<input type="text" name="menuId"  value="${errMemu.entity.menuId}" id="menuId_input" class="text_input"/>
								</td>
							</tr>
							<tr class="err_row">
								<td colspan="2">
									<font color="red" id="err_menuId">
										<c:if test="${errMemu.code=='menuId'}">
											${errMemu.msg}
										</c:if>
									</font>
								</td>
							...
							<tr class="table_tr">
								...
								<td >
									<input type="text" name="menuName"  value="${errMemu.entity.menuName}" id="menuName_input" class="text_input"/>
								</td>
							</tr>
							<tr class="err_row">
								<td colspan="2">
									<font color="red" id="err_menuName">
										<c:if test="${errMemu.code=='menuName'}">
											${errMemu.msg}
										</c:if>
									</font>
								</td>
							...
							<tr class="table_tr">
								...
								<td >
									<input type="text" name="menuURL"  value="${errMemu.entity.menuURL }" id="menuURL_input" class="text_input"/>
								</td>
							</tr>
							<tr class="err_row">
								<td colspan="2">
									<font color="red" id="err_menuURL">
										<c:if test="${errMemu.code=='menuURL'}">
											${errMemu.msg}
										</c:if>
									</font>
								</td>
								...
								<td >
									<select name="parentMenuId" id="parentMenuId_input" >
										<c:forEach var="perMenu" items="${parentMenu}">
											<option value="${perMenu.menuId}" ${perMenu.menuId==errMemu.entity.parentMenuId?"selected='selected'":"" }>${perMenu.menuName }</option>
										</c:forEach>
									</select>
								</td>
							</tr>
							<tr class="err_row">
								<td colspan="2">
									<font color="red" id="err_parentMenuId">
										<c:if test="${errMemu.code=='parentMenuId'}">
											${errMemu.msg}
										</c:if>
									</font>
								</td>
		...
		<c:if test="${errMemu.code=='errAddMenu'}">
			${errMemu.msg}
		</c:if>
		${exit}<!--退出登录部分-->
	</body>
